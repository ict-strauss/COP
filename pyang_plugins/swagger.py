...
class SwaggerPlugin(plugin.PyangPlugin):
    ...
    def add_opts(self, optparser):
        optlist = [
            optparse.make_option("--swagger-help",
                                 dest="swagger_help",
                                 action="store_true",
                                 help="Print help on tree symbols and exit"),
            optparse.make_option("--swagger-depth",
                                 type="int",
                                 dest="depth",
                                 default=5,
            ...
            optparse.make_option("--swagger-filename",
                                 dest="filename",
                                 default='service_call.json',
                                 help="Subtree to print"),
            optparse.make_option("--swagger-path",
                                 dest="path",
                                 type="string",
...
def print_header(module, fd):
    ...
    header = OrderedDict()
    header["swagger"] = "2.0"
    header["info"] = { "description": str(module_name + "API generated from service-call.yang") , "version": "1.0.0", "title": str(module_name + " API") }
    ...
    for module in modules:
        ...
        if module.i_groupings:
            for group in module.i_groupings:
                module.i_groupings[group].keyword = safety_sntax_check(group).capitalize()
                # print module.i_groupings[group].keyword

        groupings = [module.i_groupings[group] for group in module.i_groupings]
...
'400':{'description':'Invalid ID parameter'}, '404':{'description':'Call not found'}}
...
def generateRETRIEVE(ch, is_list, schema, path):
    ...
    # # Responses
    response = {'200':{'description':'Successful operation', 'schema': schema},
    '400':{'description':'Invalid ID parameter'}, '404':{'description':'' + str(ch.arg).capitalize() + ' not found'}}
...
def generateDELETE(ch, is_list, ref, path):
    ...
    # # Responses
    response = {'200':{'description':'Successful operation'}, '400':{'description':'Invalid ID parameter'}, '404':{'description':'' + str(ch.arg).capitalize() + ' not found'}}
...
def generateAPIHeader(ch, struct, operation):
    struct['summary'] = str(operation) + ' ' + str(ch.arg).capitalize() + ' by ID'
    struct['description'] = str(operation) + ' operation of resource :' + str(ch.arg)
    struct['operationId'] = str(operation).lower() + '' + str(ch.arg).capitalize() + 'ById'
    struct['produces'] = []
